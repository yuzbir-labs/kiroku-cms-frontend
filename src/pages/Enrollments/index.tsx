...
const Enrollments: React.FC = () => {
  ...
  const [courseGroupFilter, setCourseGroupFilter] = useState<number | undefined>();
  ...
  const { data: paymentsData, isLoading: paymentsLoading } = usePaymentsByEnrollmentQuery(
    selectedEnrollmentForPayments?.id || 0,
    {}
  );
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (!editingEnrollment && values.course_group) {
        const selectedGroup = courseGroups?.find((g) => g.id === values.course_group);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'enrollment_date',
      render: (date: string) => (date ? new Date(date).toLocaleDateString('az-AZ') : '-'),
    },
    {
          ...
          {record.status === 'ACTIVE' && (
            <>
              <Popconfirm
                ...
                cancelText="Xeyr"
              >
                <Button type="link" icon={<CheckCircleOutlined />} size="small" title="Tamamla" />
              </Popconfirm>
              <Popconfirm
      ...
      <FilterPanel
        filters={{
          ...
          courseGroup: {
            ...
            onChange: (value) => setCourseGroupFilter(value as number | undefined),
          ...
          status: {
            ...
            placeholder: 'Status',
            value: statusFilter,
            onChange: (value) => setStatusFilter(value as EnrollmentStatus | undefined),
            options: [
      ...
      >
        <Form form={form} layout="vertical" className={styles.form}>
            ...
            <>
              ...
              >
                <Select
                  ...
                  onChange={(value) => {
                    const selectedGroup = courseGroups?.find((g) => g.id === value);
                    if (selectedGroup) {
                      form.setFieldValue('monthly_price', selectedGroup.monthly_price);
                    }
                  }}
                />
              </Form.Item>
          ...
          {selectedStatus === 'ACTIVE' && (
            ...
            >
              <DatePicker style={{ width: '100%' }} format="DD.MM.YYYY" placeholder="Tarix seçin" />
            </Form.Item>
          )}
                ...
                <Alert
                  ...
                  description={
                    <Space direction="vertical" size="small">
                      <div>
                        <strong>Kurs:</strong> {selectedEnrollmentForPayments.course_name}
                      </div>
                      <div>
                        <strong>Qrup:</strong> {selectedEnrollmentForPayments.course_group_name}
                      </div>
                      <div>
                        <strong>Aylıq qiymət:</strong> {selectedEnrollmentForPayments.monthly_price}{' '}
                        AZN
                      </div>
                      <div>
                        <strong>Status:</strong>{' '}
                        <Tag color={statusColors[selectedEnrollmentForPayments.status]}>
              ...
              <>
                <div style={{ marginBottom: '16px' }}>
                  <Space size="large">
                    ...
                    <div>
                      <strong>Ödəniş sayı:</strong> {paymentsData.results.length}
                    </div>
                  </Space>
                </div>
                <Table
